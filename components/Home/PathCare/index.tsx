...
import { PATH_ASSETS, PATH_TEXT_CONTENT } from "@/constants/pathConstants";
  ...
  return (
    <section id="who-we-are" className="relative bg-[#130D55] overflow-hidden min-h-screen lg:min-h-[700px] flex flex-col lg:flex-row lg:items-center pt-10 pb-0 lg:py-0">
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12 w-full z-20 flex flex-col">
        ...
        >
          {/* Render dynamic title or fallback to original */}
            ...
            {title ? (
              ...
              <>
                <span className="bg-[#4C86FF] text-[#fff] px-4 py-1 rounded-lg">
                  {PATH_TEXT_CONTENT.fallbackTitleBrand}
                </span>{" "}
                {PATH_TEXT_CONTENT.fallbackTitleText}
              </>
            )}
          </h2>

          {PATH_TEXT_CONTENT.paragraphs.map((paragraph, idx) => (
            <div key={idx}>
              <p className="text-white text-[23px] font-lexend">
                {paragraph}
              </p>
              {idx < PATH_TEXT_CONTENT.paragraphs.length - 1 && (
                <>
                  <br/>
                  <br/>
                </>
          ...
          <div className="flex justify-left mt-10">
            <Link href={PATH_TEXT_CONTENT.ctaButtonHref} className="inline-block">
              <button className="bg-[#40E2B8] text-[#07004C] font-lexendBold text-[20px] px-8 py-2 rounded-full hover:bg-[#2bc49d] transition-all tracking-wide">
                {PATH_TEXT_CONTENT.ctaButtonText}
              </button>
            </Link>
          </div>
        </motion.div>
