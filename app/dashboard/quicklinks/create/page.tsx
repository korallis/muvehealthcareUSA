"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import * as z from "zod";
import {
  ArrowLeft,
  Tag,
  Link as LinkIcon,
  FileText,
  Loader2,
  Download,
  ChevronDown,
} from "lucide-react";
import Link from "next/link";
import { createQuicklinkAction } from "../actions";
import {
  createQuicklinkCategoryAction,
  getQuicklinkCategoriesAction,
} from "../../categories/quicklinks/quicklink-category-actions";
import InlineCategoryCreateDialog from "@/components/dashboard/InlineCategoryCreateDialog";

const schema = z.object({
  question: z.string().min(3, "Title is required"),
  answer: z.string().min(5, "Details are required"),
  categoryId: z.string().uuid("Please select a valid category"), // Now validating UUID
  downloadUrl: z.string().optional().or(z.literal("")),
});

type QuicklinkFormValues = z.infer<typeof schema>;

export default function CreateQuicklink() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [categories, setCategories] = useState<{ id: string; name: string }[]>(
    [],
  );

  async function loadCategories() {
    const res = await getQuicklinkCategoriesAction();
    if (res.success && res.data) {
      setCategories(res.data);
    }
  }

  useEffect(() => {
    loadCategories();
  }, []);

  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<QuicklinkFormValues>({
    resolver: zodResolver(schema),
    defaultValues: {
      categoryId: "",
      downloadUrl: "",
      question: "",
      answer: "",
    },
  });

  const onSubmit = async (data: QuicklinkFormValues) => {
    setLoading(true);
    const res = await createQuicklinkAction(data);
    if (res.success) {
      router.push("/dashboard/quicklinks");
      router.refresh();
    } else {
      setLoading(false);
    }
  };

  return (
    <div className="max-w-4xl mx-auto py-10 px-6">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/dashboard/quicklinks">
          <ArrowLeft className="text-[#1F3154] hover:scale-110 transition-transform" />
        </Link>
        <div>
          <h1 className="text-3xl font-black text-[#1F3154]">New Quicklink</h1>
          <p className="text-gray-500">
            Fill in the boxes below to publish a new resource.
          </p>
        </div>
      </div>

      <form
        onSubmit={handleSubmit(onSubmit)}
        className="bg-white p-8 rounded-3xl border shadow-xl space-y-6"
      >
        {/* UUID-based Category Dropdown */}
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="font-bold text-[#1F3154] text-sm uppercase flex items-center gap-2">
              <Tag size={14} /> Category
            </label>
            {/* <InlineCategoryCreateDialog
              createAction={createQuicklinkCategoryAction}
              onCreated={(cat) =>
                setCategories((prev) =>
                  [...prev, cat].sort((a, b) => a.name.localeCompare(b.name))
                )
              }
            /> */}
          </div>

          <div className="relative">
            <select
              {...register("categoryId")}
              className={`w-full p-3 rounded-xl border ${errors.categoryId ? "border-red-500" : "border-gray-200"} focus:ring-2 focus:ring-[#00D9DA] outline-none bg-white appearance-none cursor-pointer text-[#1F3154]`}
            >
              <option value="">Select a category...</option>
              {categories.map((cat) => (
                <option key={cat.id} value={cat.id}>
                  {cat.name}
                </option>
              ))}
            </select>
            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400">
              <ChevronDown size={16} />
            </div>
          </div>
          {errors.categoryId && (
            <p className="text-red-500 text-xs font-medium">
              {errors.categoryId.message}
            </p>
          )}
        </div>

        {/* Title */}
        <div className="space-y-2">
          <label className="font-bold text-[#1F3154] text-sm uppercase flex items-center gap-2">
            <LinkIcon size={14} /> Resource Title
          </label>
          <input
            {...register("question")}
            placeholder="e.g. Crisis Support Guide"
            className="w-full p-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#00D9DA] outline-none"
          />
          {errors.question && (
            <p className="text-red-500 text-xs font-medium">
              {errors.question.message}
            </p>
          )}
        </div>

        {/* Description */}
        <div className="space-y-2">
          <label className="font-bold text-[#1F3154] text-sm uppercase flex items-center gap-2">
            <FileText size={14} /> Description
          </label>
          <textarea
            {...register("answer")}
            rows={4}
            placeholder="Describe what this resource provides..."
            className="w-full p-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#00D9DA] outline-none"
          />
          {errors.answer && (
            <p className="text-red-500 text-xs font-medium">
              {errors.answer.message}
            </p>
          )}
        </div>

        {/* Dedicated Download URL Box */}
        <div className="p-6 bg-[#00D9DA]/5 rounded-2xl border-2 border-dashed border-[#00D9DA]/20 space-y-3">
          <label className="font-bold text-[#1F3154] text-sm uppercase flex items-center gap-2">
            <Download size={16} className="text-[#00D9DA]" /> Add Download
            Button (Optional)
          </label>
          <input
            {...register("downloadUrl")}
            placeholder="Paste the link here (e.g. website.com)"
            className="w-full p-3 rounded-xl border border-white bg-white shadow-sm focus:ring-2 focus:ring-[#00D9DA] outline-none"
          />
          <p className="text-[10px] text-gray-400 font-medium italic">
            If you paste a link here, a "Download Resource" button will
            automatically appear on the website.
          </p>
        </div>

        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#1F3154] hover:bg-[#1F3154]/90 text-white p-4 rounded-2xl font-bold flex justify-center gap-2 disabled:opacity-50 transition-all shadow-lg"
        >
          {loading ? <Loader2 className="animate-spin" /> : "Publish Quicklink"}
        </button>
      </form>
    </div>
  );
}
