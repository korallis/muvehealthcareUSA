...
import { createQuicklinkCategoryAction } from "../quicklink-category-actions";
...
export default function CreateQuicklinkCategoryPage() {
  ...
  async function onSubmit(data: CategoryFormValues) {
    ...
    // Using the specific Quicklink category action
    const result = await createQuicklinkCategoryAction(data);

    if (result && "error" in result) {
      setServerError(result.error);
      setLoading(false);
    } else {
      // Returns the user to the Quicklink creation form
      router.back();
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-black text-[#1F3154] tracking-tight">
            New Quicklink Category<span className="text-[#00D9DA]">.</span>
          </h1>
          <p className="text-gray-500">
            Create a specific category to group your resources and files.
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl shadow-gray-200/50 space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-[#1F3154] uppercase tracking-wider flex items-center gap-2">
              <Tag size={14} /> Quicklink Category Name
            </label>
            <input
              {...register("name")}
              placeholder="e.g. Policy Documents, Crisis Support, Financial Help"
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Briefly describe the types of files or links in this group..."
